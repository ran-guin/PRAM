...
router.get('/:id', function (req, res) {
    ...
    var Config = { user : user, userid : userid, url: URL, recordId : orderId, itemClass : 'Item_Request'};
...
router.get('/search', function(req, res){
	...
	var url = URL + "/pram/items/search/" + val;
...
router.post('/create', jsonParser, function (req,res) {
    console.log('Posting Requisition');
    if (! req.body) { 
        console.log('Error parsing body');
        return res.sendStatus(400);
    }
    ...
    dbc.beginTransaction(function(err) {
        ...
        for (k in obj) {
            ...
            else if (map && map[k]) { insertR[map[k]] = obj[k]; info++ } // use mapped field name
            else { insertR[k] = obj[k]; info++; }                  // use key if no mapping supplied
        } 
        ...
        if (info) {
            ...
            dbc.query(q, insertR, function(err, result) {
                if (err || ! result.insertId) { 
                    console.log('Insert Requisition Error: ' + err);
                    throw err;
                }
                ...
                dbc.commit(function(err) {
                    ...
                    console.log('successful commit of requisition ' + obj.orderId );
                   var returnval = { 'Record_ID' : obj.orderId, 'Description' : obj.description };
                   res.send( returnval );
                });
            });               
        }   
        else {
            console.log('no insert data');
            res.send();
        }
