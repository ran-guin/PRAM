...
/* 
router.get('/search/:table', function(req, res){
	var table = req.params.table;
	var search = req.query.search;
    	console.log('Search ' + table + ' Records');
    ...
    db.waterfall([
	    ...
	    console.log('insert #1');
            dbc.query("INSERT INTO Request (Request_Notes) VALUES ('?')", 'Test Async', cb);
        },
        function (dbc, results, cb) {
            var table1_id = results.insertId;
            var values = [ table1_id, 'Related Async Record' ];
	    console.log('insert #2');

            dbc.query("INSERT INTO Item_Request (FK_Request__ID, Notes) VALUES ($1, $2)", values, cb);
        },
    ], 
    ...
    async.series([
		...
		console.log('Connected to Database');
		track['one'] = 'A';
                asyncCallback(null, dbc);
        },
        function(asyncCallback){
                // do some more stuff ...a
//		var timestamp = new Date().toISOString();
//		var values = ['Test Async', '2015-04-01'];
		console.log('do second task')
		dbc.query("INSERT INTO Request (Request_Notes) VALUES (?)", 'Test Async', function ( err, result ) { 
			track['two'] = 'B';
			if (err || !result.insertId) { asyncCallback(err) }			
			else { asyncCallback(null, result.insertId) }
		}) ;
         },
	function (asyncCallback) {
		console.log('task 3: ');
		asyncCallback(null,track['two']);	
	} 
    ],
    function cb (err, results) {
	if (err) { console.log("ASYNC Error1: " + err) }
    ...
    var Fields = [ { 'Name' : 'ID', 'Type' : 'INT'}, { 'Name' : 'Name', 'Type' : 'TEXT'}];
    res.render('searchRecords', { table : table, fields : Fields });
